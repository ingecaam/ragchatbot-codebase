Refactor @backend/ai_generator.py to support sequential tool calling where 
Claude can make up to 2 tool calls in separate API rounds. 
...
previous results 
- Support for complex queries requiring multiple searches for comparisons, multi
part questions, or when information from different courses/lessons is needed 
...
Use two parallel subagents to brainstorm possible plans. Do not implement any 
code.